...
import { allbookings } from "../../api/adminapi";
...
const BookingTable=() => {
    ...
    const { isLoading, data: Data, isSuccess } = useQuery({
        queryKey: ['allData', page, limit],
        queryFn: () => allbookings(page, limit),
    });
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {Data?.data.map((item:Booking, index: number) => (
                        <tr key={index}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-center">
                                <div className="text-sm text-gray-900">{item.hostId?.fname} {item.hostId?.lname}</div>
                            </td>
                        </tr>
                    ))}
                </tbody>
